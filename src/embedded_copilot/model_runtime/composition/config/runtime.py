...
def load_model_runtime_config(settings: ModelSettingsSource) -> ModelRuntimeConfig:
    ...
    try:
        model = _model_identifier(settings.ollama_model)
        base_url = _validated_base_url(
            settings.ollama_base_url,
            require_tls=settings.ollama_require_tls,
        )
    except (TypeError, ValueError) as error:
        raise ValueError("model runtime configuration is invalid") from error
    return ModelRuntimeConfig(
        provider="ollama",
        base_url=base_url,
        model=model,
...
def _validated_base_url(value: object, *, require_tls: bool) -> str:
    ...
    try:
        ...
        raise ValueError("model endpoint is invalid") from error
