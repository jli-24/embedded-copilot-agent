...
_UNSAFE = re.compile(
    ...
    r"serial(?:\s+number)?|mac(?:\s+address)?|com\d*|baudrate|raw\s+bytes|"
    r"stdout|stderr|stack\s+trace|command|binary|firmware\s+image|"
    r"test\s+script|environment)",
    re.IGNORECASE,
)
...
def safe_text(value: object, *, field: str, maximum: int = 512, allow_empty: bool = False) -> str:
    if not isinstance(value, str):
        raise ValueError(f"{field} is invalid")
    text = unicodedata.normalize("NFC", value.strip())
    if (
