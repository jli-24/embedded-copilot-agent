...
class HardwareKnowledgeRetriever:
    """Deterministic in-memory hardware keyword retriever."""

    def __init__(
        self,
        documents: Sequence[HardwareDocument] | None = None,
        *,
        top_k: int = 4,
    ) -> None:
        if top_k <= 0:
    ...
    def add_documents(self, documents: Sequence[HardwareDocument]) -> None:
        for document in documents:
            self._documents[document.id] = document

    def search(self, query: str) -> Sequence[HardwareDocument]:
        return self.retrieve(query)

    def retrieve(self, query: str) -> list[HardwareDocument]:
        query_tokens = _tokens(query.strip())
        if not query_tokens:
            return []

        ranked: list[tuple[int, int, HardwareDocument]] = []
